...
(()=>{'use strict';
  ...
  function appsSignature(list){
    return list
      .map(app=>[
        app.id,
        app.status,
        app.updated_at||app.reviewed_at||app.created_at||'',
        app.rejection_reason||''
      ].join(':'))
      .join('|');
  }

  async function findApplication(id,refresh=false){
    let app=adminApplications.find(x=>String(x.id)===String(id));
    if(app||!refresh)return app||null;

    await loadAdminApplications(true);
    app=adminApplications.find(x=>String(x.id)===String(id));
    return app||null;
  }
  ...
  function openDialog(dialog){
    ...
    dialog.classList.add('nx156-dialog-fallback');
  }
  ...
  function closeDialog(dialog){
    ...
    dialog.classList.remove('nx156-dialog-fallback');
  }
  ...
  function injectAdminPanels(){
    ...
    if(alertsScreen()){
      ...
      const signature=appsSignature(list);
      let panel=main.querySelector('[data-nx154-panel="alerts"]');

      if(!panel){
        main.insertAdjacentHTML('afterbegin',adminPanelMarkup('alerts'));
        panel=main.querySelector('[data-nx154-panel="alerts"]');
        if(panel)panel.dataset.nx156Signature=signature;
        lastAlertsSignature=signature;
      }else if(panel.dataset.nx156Signature!==signature || lastAlertsSignature!==signature){
        const next=document.createElement('div');
        next.innerHTML=adminPanelMarkup('alerts');
        const replacement=next.firstElementChild;
        replacement.dataset.nx156Signature=signature;
    ...
    if(sellersScreen()){
      ...
      if(!panel){
        ...
        panel=main.querySelector('[data-nx154-panel="sellers"]');
        if(panel)panel.dataset.nx156Signature=signature;
        lastSellersSignature=signature;
      }else if(panel.dataset.nx156Signature!==signature || lastSellersSignature!==signature){
        const next=document.createElement('div');
        next.innerHTML=adminPanelMarkup('sellers');
        const replacement=next.firstElementChild;
        replacement.dataset.nx156Signature=signature;
        panel.replaceWith(replacement);
        lastSellersSignature=signature;
  ...
  async function reviewApplication(id,decision,reason=''){
    if(!canAdmin()||!client()){
      toast('صلاحية الإدارة غير جاهزة. أعد فتح لوحة الإدارة وحاول مرة أخرى.','error');
      return false;
    }

    const app=adminApplications.find(x=>String(x.id)===String(id));
    if(!app){toast('طلب الانضمام غير موجود','error');return false}

    if(decision==='rejected' && String(reason||'').trim().length<3){
      toast('اكتب سبب الرفض أولًا','error');return false;
    }
    ...
    try{
      ...
      closeDialog($('#nx154SellerDetails'));
      closeDialog($('#nx154RejectDialog'));
  ...
  function observeSurfaces(){
    ...
    if(moduleMain){
      new MutationObserver(records=>{
        // Ignore mutations caused only by our own seller panels.
        const meaningful=records.some(record=>{
          ...
          return changed.some(node=>
            !node.matches?.('[data-nx154-panel]') &&
            !node.closest?.('[data-nx154-panel]')
          );
        });
      ...
      new MutationObserver(records=>{
        const meaningful=records.some(record=>
          [...record.addedNodes,...record.removedNodes].some(node=>
            node?.nodeType===1 &&
            !node.matches?.('.nx154-account-status') &&
            !node.closest?.('.nx154-account-status')
          )
        );
  ...
  function bindEvents(){
    document.addEventListener('click',event=>{
      ...
      const openBell=target.closest('[data-action="open-alerts"]');
      if(openBell){
      ...
      const details=target.closest('[data-nx154-details]');
      if(details){
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();

        const id=details.dataset.nx154Details;
        (async()=>{
          const app=await findApplication(id,true);
          if(!app)return toast('تعذر العثور على طلب الانضمام','error');
          detailsDialog(app);
        })();
        return;
      }

      const approve=target.closest('[data-nx154-approve]');
      if(approve){
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();

        const id=approve.dataset.nx154Approve;
        approve.disabled=true;
        approve.textContent='جارٍ القبول…';

        (async()=>{
          const app=await findApplication(id,true);
          if(!app){
            toast('تعذر العثور على طلب الانضمام','error');
            approve.disabled=false;
            approve.textContent='قبول';
            return;
          }
      ...
      const reject=target.closest('[data-nx154-reject]');
      if(reject){
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();

        const id=reject.dataset.nx154Reject;
        (async()=>{
          const app=await findApplication(id,true);
          if(!app)return toast('تعذر العثور على طلب الانضمام','error');
          rejectionDialog(app);
      ...
      const close=target.closest('[data-nx154-close]');
      if(close){
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
      ...
      if(target.closest('[data-action="customer-account"]')){
        setTimeout(()=>{
          myApplication=null;
          loadMyApplication(true);
        },180);
      ...
      const legacyReject=target.closest('[data-seller-application="rejected"]');
      if(legacyReject){
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
    ...
    document.addEventListener('submit',event=>{
      const target=event.target instanceof Element?event.target:null;
      const form=target?.closest('[data-nx154-reject-form]');
      if(!form)return;
