...
-- =====================================================================
-- V15.5 PUBLIC CONTACT SETTINGS
-- Exposes ONLY the site's public WhatsApp contact number.
-- =====================================================================
begin;

create or replace function public.get_public_contact_settings_v155()
returns jsonb
language sql
stable
security definer
...
as $$
  select jsonb_build_object(
    ...
    coalesce(
      (
        select state_data->'settings'->>'whatsappNumber'
...
revoke all on function public.get_public_contact_settings_v155() from public;
grant execute on function public.get_public_contact_settings_v155() to anon, authenticated;

commit;

select
  'nuvexa_cross_browser_seller_fix_v15_5' as migration,
  'READY' as status,
  coalesce(
    (
      select state_data->'settings'->>'whatsappNumber'
